...
public class ControllerRCS {
    ...
    @CrossOrigin(origins = httpsUrl, allowedHeaders = allowed_headers)
    @PreAuthorize(PERMIT_ALL)
    @PostMapping(value = ConstantsHttps.GET_JSON_MODELS_RCS)
    Response addJsonModelsRCS(){
        return rcs.addJsonModelsRCS();
    }
//    @CrossOrigin(origins = httpsUrl, allowedHeaders = allowed_headers)
//    @PreAuthorize(PERMIT_ALL)
//    @PostMapping(value = ConstantsHttps.GET_JSON_MODELS_RCS)
//    Response addJsonModelsRCS(@RequestBody ModelRCS[] modelsRCS){
//        return rcs.addJsonModelsRCS(modelsRCS);
//    }
